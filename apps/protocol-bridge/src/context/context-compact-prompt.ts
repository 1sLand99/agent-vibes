export type ContextCompactionMode = "full" | "from" | "up_to"

export const CONTEXT_COMPACT_MAX_OUTPUT_TOKENS = 20_000
...
export function formatContextCompactSummary(raw: string): string {
  if (!raw) return ""
  const summaryStart = /<summary>/i.exec(raw)
  let formatted: string
  if (summaryStart) {
    const contentStart = summaryStart.index + summaryStart[0].length
    const tail = raw.slice(contentStart)
    const summaryEnd = /<\/summary>/i.exec(tail)
    formatted = summaryEnd ? tail.slice(0, summaryEnd.index) : tail
  } else {
    formatted = raw
      .replace(/<analysis>[\s\S]*?<\/analysis>/gi, "")
      .replace(/<analysis>[\s\S]*$/gi, "")
  }
  // The bridge stores the summary body in ContextCompactionCommit.summary;
  // renderCompactSummary supplies the user-facing context header. Adding
  // Claude Code's `Summary:` display header here would duplicate that layer.
